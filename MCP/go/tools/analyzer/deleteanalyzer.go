...
func DeleteanalyzerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/analyzer/%s%s", cfg.BaseURL, analyzerName, queryString)
...
func CreateDeleteanalyzerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_analyzer_analyzerName",
		mcp.WithDescription("Deletes the specified analyzer. When you delete an analyzer, IAM Access Analyzer is disabled for the account or organization in the current or specific Region. All findings that were generated by the analyzer are deleted. You cannot undo this action."),
		...
		mcp.WithString("analyzerName", mcp.Required(), mcp.Description("The name of the analyzer to delete.")),
		mcp.WithString("clientToken", mcp.Description("A client token.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteanalyzerHandler(cfg),
	}
}
