...
func DeletearchiveruleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/analyzer/%s/archive-rule/%s%s", cfg.BaseURL, analyzerName, ruleName, queryString)
...
func CreateDeletearchiveruleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_analyzer_analyzerName_archive-rule_ruleName",
		mcp.WithDescription("Deletes the specified archive rule."),
		...
		mcp.WithString("analyzerName", mcp.Required(), mcp.Description("The name of the analyzer that associated with the archive rule to delete.")),
		mcp.WithString("ruleName", mcp.Required(), mcp.Description("The name of the rule to delete.")),
		mcp.WithString("clientToken", mcp.Description("A client token.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletearchiveruleHandler(cfg),
	}
}
