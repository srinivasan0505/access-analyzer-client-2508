...
func ListarchiverulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/analyzer/%s/archive-rule%s", cfg.BaseURL, analyzerName, queryString)
		...
		var result models.ListArchiveRulesResponse
...
func CreateListarchiverulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_analyzer_analyzerName_archive-rule",
		mcp.WithDescription("Retrieves a list of archive rules created for the specified analyzer."),
		...
		mcp.WithString("analyzerName", mcp.Required(), mcp.Description("The name of the analyzer to retrieve rules from.")),
		mcp.WithString("nextToken", mcp.Description("A token used for pagination of results returned.")),
		mcp.WithNumber("maxResults", mcp.Description("The maximum number of results to return in the request.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListarchiverulesHandler(cfg),
	}
}
