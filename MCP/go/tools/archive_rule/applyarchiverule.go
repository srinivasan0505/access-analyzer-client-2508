...
func ApplyarchiveruleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ApplyArchiveRulerequest
		...
		url := fmt.Sprintf("%s/archive-rule", cfg.BaseURL)
...
func CreateApplyarchiveruleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_archive-rule",
		mcp.WithDescription("Retroactively applies the archive rule to existing findings that meet the archive rule criteria."),
		...
		mcp.WithString("ruleName", mcp.Required(), mcp.Description("Input parameter: The name of the rule to apply.")),
		mcp.WithString("analyzerArn", mcp.Required(), mcp.Description("Input parameter: The Amazon resource name (ARN) of the analyzer.")),
		mcp.WithString("clientToken", mcp.Description("Input parameter: A client token.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ApplyarchiveruleHandler(cfg),
	}
}
