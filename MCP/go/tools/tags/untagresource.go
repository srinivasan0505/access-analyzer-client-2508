...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_tags_resourceArn#tagKeys",
		mcp.WithDescription("Removes a tag from the specified resource."),
		...
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description("The ARN of the resource to remove the tag from.")),
		mcp.WithArray("tagKeys", mcp.Required(), mcp.Description("The key for the tag to add.")),
	)
