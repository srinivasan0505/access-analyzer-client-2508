...
func ListpolicygenerationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["principalArn"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("principalArn=%v", val))
		}
		...
		url := fmt.Sprintf("%s/policy/generation%s", cfg.BaseURL, queryString)
		...
		var result models.ListPolicyGenerationsResponse
...
func CreateListpolicygenerationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_policy_generation",
		mcp.WithDescription("Lists all of the policy generations requested in the last seven days."),
		...
		mcp.WithString("principalArn", mcp.Description("The ARN of the IAM entity (user or role) for which you are generating a policy. Use this with <code>ListGeneratedPolicies</code> to filter the results to only include results for a specific principal.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListpolicygenerationsHandler(cfg),
	}
}
